...
YOLOv5_Detector::YOLOv5_Detector(Net_config config)
{
...
//    modelFile += ".onnx";
    modelFile = "F:/Work and Learn/Projects/QtProject/PedestrianCounting-WarningSystem/yolov5s.onnx";
    this->net = readNet(modelFile);
}

void YOLOv5_Detector::drawPred(int classId, float conf, int left, int top, int right, int bottom, Mat& frame)   // Draw the predicted bounding box
{
...
void YOLOv5_Detector::sigmoid(Mat* out, int length)
{
...
int YOLOv5_Detector::detect(Mat& frame)
{
    ...
    cout << "There are " << indices.size() << "persons" << endl;    // 输出检测的目标个数
